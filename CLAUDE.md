...
**Response language**: Always respond in **English**. No translations, no language switching — plain English only.
...
- Provide advanced features like AI song suggestions, synced lyrics, animated artwork
...
- **Native Sonos Wi-Fi Speaker Support (2026-09-25)**: Spotify-style native Sonos speaker integration (`com.maxrave.media3.sonos`). Uses SSDP multicast discovery (`239.255.255.250:1900` with `WifiManager.MulticastLock`) to find Sonos ZonePlayers on the local Wi-Fi network without external cloud APIs. Controls playback and volume via UPnP SOAP (`AVTransport:1` and `RenderingControl:1` on port 1400). Streams audio through an embedded lightweight local HTTP proxy server (`SonosAudioServer` on `ServerSocket(0)`) that handles `Range` requests (`206 Partial Content` for instant scrubbing/seeking), serves DIDL-Lite track/artwork metadata, and automatically refreshes expired/403 YouTube stream URLs transparently. `SonosHandoffManager` coordinates seamless handoff with `CrossfadeExoPlayerAdapter` via `SonosPlayer` (Media3 `ForwardingPlayer` with `DeviceInfo.PLAYBACK_TYPE_REMOTE`), routing play/pause, skip next/previous, seeking, queue advancement, and phone hardware volume buttons (`MainActivity.onKeyDown` + remote `MediaSession`). Exposed through `DevicePickerBottomSheet` and the Now Playing speaker pill banner in `ReferenceNowPlayingLayout`.
- **Unified Liked Songs + account library caching + playlist audio/video tags (2026-08-19)**: every Liked Songs entry point (Home hero carousel, quick picks, dynamic sections, offline error state, Library "Favorite" chip) now opens the canonical YouTube Music Liked Songs playlist (`PlaylistDestination("LM", isYourYouTubePlaylist = true)`) instead of the local-DB `LibraryDynamicPlaylistType.Favorite` screen; `PlaylistViewModel` no longer overwrites the online `LM` track list with local liked songs. `PlaylistScreen` gained Material 3 **All / Songs / Videos** filter chips backed by `PlaylistTag` + `Track.isVideoTrack()` (OMV/UGC/`"Videos"` = video; ATV/null/`"Song"` = song), and Play All / item-click / shuffle respect the active tag. Track classification is now real: `MusicResponsiveListItemRenderer.musicVideoType` reads `watchEndpointMusicConfig` (also overlay/flex-column fallbacks) and `PlaylistParser`/`SongItem.toTrack()` propagate it instead of the hardcoded `videoType = "video"`. Library Albums/Artists moved from SQLite queries to YouTube Music account scrapers (`FEmusic_liked_albums`, `FEmusic_library_corpus_track_artists` via new `getLibraryAlbums`/`getLibraryArtists`, parsed by `LibraryAlbumArtistParser`), with offline-first caching through new `libraryPlaylistsCache`/`libraryAlbumsCache`/`libraryArtistsCache` DataStore keys — the UI paints the cached list instantly, then refreshes from the account in the background (`AlbumsResult`/`ArtistsResult`/`PlaylistsResult` are now `@Serializable`, and `AlbumsResult.duration` was tightened from `Any` to `String`). `CustomCoverHelper` canonicalizes Liked Songs aliases (`LM`/`VLLM`/`FEmusic_liked_videos`/`favorite_songs`/`liked_songs`) so custom covers set under any alias resolve (and remove) across backup/restore and device migrations.
- **Apple Music-style now playing background (2026-08-14)**: `ReferenceBackdrop` (player/lyrics/queue tabs) swapped the Haze frosted-artwork wash for `AppleMusicBackground` — a twist+blur approximation of Apple's Metal shader: up to 3 copies of the artwork, each scaled 3x, rotated around a slowly drifting centre (13-21s linear cycles, `rememberInfiniteTransition`) and heavily blurred via the common `androidx.compose.ui.graphics.BlurEffect` (GPU `RenderEffect` on Android, Skia ImageFilter on desktop). Scrim: PLAYER darker (0.35→0.75), LYRICS/QUEUE brighter (0.15→0.55), crossfaded on tab switch. Button colours come from the new `Palette.toAppleMusicTintColor()` — the artwork's MOST SATURATED swatch, with saturation quartered and brightness maxed — applied as translucent tints (primary 30% alpha, secondary 20% alpha) so controls read as frosted glass, never solid colour; ALL icons are white unless a button is SELECTED, in which case the button turns solid white with a dark icon (bottom/queue segments, liked heart). Performance fatigue protection (`PerformanceMonitor.kt`): a 3s sliding window of `withFrameNanos` deltas flags >22ms average frames, OR'd with an Android platform signal (power-save broadcast + 5s thermal poll — `ACTION_THERMAL_STATUS_CHANGED` is @SystemApi so it is polled, not broadcast) that makes the background static (single blurred fill) until the device recovers; the whole thing is off via the new `animated_now_playing_background` DataStore setting. **Trap**: the generated string accessors (`Res.string.<name>`) are extension properties in the `simpmusic.composeapp.generated.resources` package and need an explicit per-string import in every file that uses them — the JVM target tolerates the omission while the Android target (K2 `-Xfragments` mode) reports "Unresolved reference" on the property name, which looks like a stale-cache/daemon bug but is just the missing import (the icons trap applies to strings too). Follow-ups: the backdrop artwork and the tab pages use fade-THROUGH transitions (delayed fade-out) so song/tab changes never dip through black; the big player artwork and the small header artwork are ONE shared element (`SharedTransitionLayout` + scope `rememberSharedContentState`, threaded via CompositionLocals because the scope receiver doesn't propagate into nested composables — call it as `with(scope) { modifier.sharedElement(state, animScope) }`) so tab switches morph the cover to its new position; the progress bar played portion + thumb are the solid tint; video tracks render inline 16:9 via `MediaPlayerViewWithSubtitle` (main player + YouTube captions) with a tap-toggle overlay (fullscreen, ±5s, captions) holding only buttons absent from the layout below.
- **Spotify Canvas removed, generic animated-artwork infra kept (2026-08-13)**: the Spotify Canvas feature is gone — `getSpotifyCanvas`/`CanvasResponse`/`CanvasBody` in `core/service/spotify`, the token-fetching `getCanvas` + `updateCanvasUrl`/`updateCanvasThumbUrl` in `LyricsRepository` (renamed from `LyricsCanvasRepository`/`LyricsCanvasRepositoryImpl`), the DataStore `spotify_canvas` flag and the Settings toggle are deleted. The generic plumbing is renamed `animatedArtwork` for the planned Apple Music animated-artwork integration: `Config.ANIMATED_ARTWORK_CACHE`, the `AnimatedArtworkResult` domain model, the fullscreen video renderer in `NowPlayingScreen`/`ArtistScreen`, `SongEntity.animatedArtworkUrl`/`animatedArtworkThumbUrl` (still mapped to the existing `canvasUrl`/`canvasThumbUrl` SQL columns — no migration), and the Settings cache row (`animated_artwork_cache`). DB schema and old column names are untouched.
